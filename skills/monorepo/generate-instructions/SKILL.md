...
description: Analyze a monorepo directory and generate agent context files. Use when creating CLAUDE.md, AGENTS.md, Cursor rules, or Copilot instructions for a directory.
---

# Generate Directory Instructions

Analyze a specific directory in the monorepo and create a context file for the user's coding agent.

## Inputs

1. **Target Directory** (required) - The directory path to analyze (e.g. `packages/api`, `apps/web`)
2. **Output format** (optional) - Infer from project or ask the user

## Output

Create a context file using one of these patterns:

| Format | Output path |
|--------|-------------|
| Claude Code | `{directory}/CLAUDE.md` |
| Codex | `{directory}/AGENTS.md` |
| Cursor | `.cursor/rules/{DIR_NAME}.mdc` with `globs: "{DIR_PATH}/**"` |
| GitHub Copilot | `.github/instructions/{DIR_NAME}.instructions.md` |

All formats include the same 7 sections populated with directory-specific information.

## Steps

1. **Analyze the Directory** - Examine code structure, patterns, and conventions
2. **Generate Instructions File** - Create the file with all sections
...
- For Cursor rules, set `globs` to match the target directory (e.g. `packages/api/**`)
