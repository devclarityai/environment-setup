---
agent: agent
description: Analyze a directory and generate consolidated GitHub Copilot instructions.
---

# Generate Directory Instructions

Analyze a specific directory in the monorepo and create a consolidated instruction file for GitHub Copilot.

## Inputs
1. **Target Directory** (required) - The directory path to analyze (e.g., `packages/api`, `apps/web`)

## Output
Create a single file at `.github/instructions/{DIR_NAME}.instructions.md` with:
- Frontmatter: `applyTo: "{DIR_PATH}/**"`
...
- The `applyTo` glob should match the target directory (e.g., `packages/api/**`)
